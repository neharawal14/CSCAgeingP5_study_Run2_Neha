void reading_periods_2016_comparison(){

  TString year = "2016";
  TString input_path = "/eos/home-n/nrawal/CSCAgeing/2024_new_plots/Comparison/";
 ...
 for(int i=0; i<chamber_list.size(); i++){
  ...
  TString file_path1 = input_path+"2016_second_period_initial/"+name;
  TString file_path2 = input_path+"2016_second_period_second_more_entries/"+name;
  TString file_path3 = input_path+"2016_second_period_third_overflow/"+name;
...
///  TString file_path4 = input_path_complete+name;
 ...
 // TFile *f4 = TFile::Open(file_path4,"READ");
  ...
  c->Divide(2,2);
  c->cd(1); 
  h1->SetTitle("initial with 100 entries const");
  h1->Draw();
  c->cd(2); 
  h2->SetTitle("without 100 entries const");
  h2->Draw();
  c->cd(3); 
  h3->SetTitle("without 100 entries const and counting overflow");
  h3->Draw();
 // c->cd(4); 
 // h4->Draw();
  c->SaveAs(year+"_comparison/gas_gain_vs_pressure_"+chamber+".pdf");

  //h4->SetMarkerStyle(20);
  //h4->SetMarkerSize(0.8);
  std::vector<TH1D*> histograms = {h1, h2, h3};
  std::vector<int> colors = {kMagenta, kBlue, kGreen}; // Define colors for each histogram
  std::vector<int> marker_types = {24, 29, 22}; // Define colors for each histogram
  std::vector<TString> periods = {"Initial", "no entry const", "counting overflow"};
...
TCanvas *c_all = new TCanvas();
 ...
 for(int i=0; i<histograms.size(); i++){
  histograms[i]->SetMarkerStyle(marker_types[i]); 
  histograms[i]->SetMarkerColor(colors[i]); 
  histograms[i]->SetTitle(chamber); 
...
//   stats->SetOptStat(1111);
  ...
  legend->Draw();
 c_all->SaveAs(year+"_comparison/gas_gain_vs_pressure_together_"+chamber+".pdf");
