void reading_periods_run2_using_big_dataset(){
 ...
 for(int i=0; i<chamber_list.size(); i++){
  ...
  TString file_path1 = input_path+year+"_first_period/"+name;
  TString file_path2 = input_path+year+"_second_period_first_half/"+name;
  TString file_path3 = input_path+year+"_third_period/"+name;
...
///  TString file_path4 = input_path_complete+name;
  ...
  c->cd(4); 
  h4->Draw();
  c->SaveAs(year+"_good_ones/gas_gain_vs_pressure_"+chamber+".pdf");
...
//   stats->SetOptStat(1111);
  ...
  legend->Draw();
 c_all->SaveAs(year+"_good_ones/gas_gain_vs_pressure_together_"+chamber+".pdf");
