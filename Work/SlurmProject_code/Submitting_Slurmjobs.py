...
parser.add_argument('--number', required=True, help='number to keep track of same dataset (e.g., B)')
...
filelist_path = f"filelist_{year}{dataset_name}_{number}.txt"
    ...
    for dirpath, dirnames, filenames in os.walk(input_path):
        for filename in filenames:
            if filename.endswith(".root"):
                ...
                filelist.write(f"{input_file} {output_file} {year} {dataset_name}\n")

print(f"Saved list of ROOT files to: {filelist_path}")
with open(f"filelist_{year}{dataset_name}_{number}.txt") as f:
    n_lines = sum(1 for _ in f)
print(" n lines ",n_lines)
os.system(f"sbatch --array=0-{n_lines - 1} --export=ALL,FILELIST=filelist_{year}{dataset_name}_{number}.txt slurm_job_script_togther.sbatch")
##with open(f"failed_logs_{year}{dataset_name}.txt") as f:
##    n_lines = sum(1 for _ in f)
##os.system(f"sbatch --array=0-{n_lines - 1} --export=ALL,FILELIST=failed_logs_{year}{dataset_name}.txt slurm_job_script_togther.sbatch")
