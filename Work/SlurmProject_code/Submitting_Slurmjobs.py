...
with open(f"failed_filelist_{year}.txt") as f:
    n_lines = sum(1 for _ in f)
os.system(f"sbatch --array=0-{n_lines - 1} --export=ALL,FILELIST=failed_filelist_{year}.txt slurm_job_script_togther.sbatch")
